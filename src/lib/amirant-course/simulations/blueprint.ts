/**
 * The simulation blueprint, derived from `exam-facts.ts`.
 *
 * Client-safe: it must never import `content-source`. The authoring-source
 * override lives in `definitions.ts`, which is build-time only — see
 * `../manifest.ts` for why.
 */
...
export const DEMO_SIMULATIONS: ManifestSimulation[] = [
