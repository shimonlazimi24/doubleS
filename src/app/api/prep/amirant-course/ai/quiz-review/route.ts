...
import { runQuizReviewAi } from "@/lib/amirant-course/ai/quiz-review";
...
export async function POST(req: Request) {
  ...
  if (!(await checkAiUserAndIpRateLimit({ userId: user.id, ip: requestIp, route: "quiz-review" }))) {
  ...
  try {
    const data = await runQuizReviewAi(client, user.id, body, { requestIp, sessionId });
    return NextResponse.json(data);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Quiz review failed";
