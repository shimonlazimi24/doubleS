    ...
    <div className={cn(shell, className)} lang="he">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        {nextLessonHref && nextLessonTitle ? (
          <Link
            href={nextLessonHref}
            className="inline-flex min-h-[2.75rem] flex-col items-stretch justify-center gap-0.5 rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-start text-sm text-gray-900 transition hover:border-primary/35 hover:text-primary"
          >
            <span className="font-medium">המשך לשיעור הבא</span>
            <span className="text-xs font-normal text-muted">{nextLessonTitle}</span>
          </Link>
        ) : null}
      ...
      <p className="mt-4 text-xs text-muted">
        מזהה שיעור: <code className="rounded bg-paper px-1 py-0.5 text-ink" translate="no">{lessonId}</code>
